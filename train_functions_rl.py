...
# 전역적으로 기울기 계산 비활성화
# torch.set_grad_enabled(False)s
...
def calculate_performance_metrics(returns_list, minimum_date=40):
    ...
    cumulative_returns = np.full(chromosomes_size, 1)  # Initialize Cumulative Returns
...
# def calculate_same(same_prob, pos_list, price_list, leverage_ratio, enter_ratio, profit, entry_pos, curr_close, additional_count, limit=2, cut_value=1., min_enter_ratio=0.05):
...
#     enter_enter_ratio = activation_fn(same_prob[enter_index][:, 5])
#     enter_enter_ratio = torch.clamp(enter_enter_ratio, min=min_enter_ratio)
...
# def calculate_diff(diff_prob, pos_list, price_list, leverage_ratio, enter_ratio, profit, entry_pos, curr_close, additional_count, min_enter_ratio=0.05):
...
#     switch_leverage = activation_fn(diff_prob[switch_index][:, 4]) * 100.
#     switch_enter_ratio = activation_fn(diff_prob[switch_index][:, 5])
#     switch_enter_ratio = torch.clamp(switch_enter_ratio, min=min_enter_ratio)
...
# def calculate_hold(hold_prob, pos_list, price_list, leverage_ratio, enter_ratio, profit, entry_pos, curr_close, additional_count, min_enter_ratio=0.05):
...
#     # enter
#     enter_leverage = activation_fn(hold_prob[enter_index][:, 4]) * 100.
#     enter_enter_ratio = activation_fn(hold_prob[enter_index][:, 5])
#     enter_enter_ratio = torch.clamp(enter_enter_ratio, min=min_enter_ratio)
...
def loss_cut_fn(pos_list, price_list, leverage_ratio, enter_ratio, profit,
    ...
    # Calculate profit or loss
    short_profit = -((curr_high - price_list[short_index]) 
                     / price_list[short_index] * 100.) * leverage_ratio[short_index]
    long_profit = ((curr_low - price_list[long_index]) 
                   / price_list[long_index] * 100.) * leverage_ratio[long_index]
    ...
    # Update state for short positions to be cut
    short_index = short_index[short_cut_index]
    profit[short_index] = profit[short_index] - (enter_ratio[short_index] * cut_percent * alpha) \
                          - 0.1 * leverage_ratio[short_index] * enter_ratio[short_index]
    pos_list[short_index] = 0
    price_list[short_index] = -1.
    leverage_ratio[short_index] = -1
    enter_ratio[short_index] = -1.
    ...
    # Update state for long positions to be cut
    long_index = long_index[long_cut_index]
    profit[long_index] = profit[long_index] - (enter_ratio[long_index] * cut_percent * alpha) \
                         - 0.1 * leverage_ratio[long_index] * enter_ratio[long_index]
    pos_list[long_index] = 0
    price_list[long_index] = -1.
    leverage_ratio[long_index] = -1
    enter_ratio[long_index] = -1.
...
def calculate_same(same_prob, pos_list, price_list, leverage_ratio, enter_ratio, profit,
                   entry_pos, curr_close, additional_count, limit=2,
                   cut_value=1., min_enter_ratio=0.05):
    # indices in same_prob
    index = torch.tensor([0, 1, 3], device=same_prob.device)
    logit = torch.argmax(same_prob[:, index], dim=1)
    hold_index = torch.where(logit == 0)[0]
    enter_index = torch.where((logit == 1) & (additional_count < limit))[0]
    loss_index = torch.where(logit == 2)[0]
    ...
    if entry_pos == 2:  # 'long' -> 2
        profit[loss_index] = profit[loss_index] - loss_profit \
                             - 0.1 * leverage_ratio[loss_index] * enter_ratio[loss_index]
    elif entry_pos == 1:  # 'short' -> 1
        profit[loss_index] = profit[loss_index] + loss_profit \
                             - 0.1 * leverage_ratio[loss_index] * enter_ratio[loss_index]
...
def calculate_diff(diff_prob, pos_list, price_list, leverage_ratio, enter_ratio, profit,
                   entry_pos, curr_close, additional_count, min_enter_ratio=0.05):
    # indices in diff_prob
    index = torch.tensor([0, 1, 2], device=diff_prob.device)
    logit = torch.argmax(diff_prob[:, index], dim=1)
    hold_index = torch.where(logit == 0)[0]
    switch_index = torch.where(logit == 1)[0]
    take_index = torch.where(logit == 2)[0]
    ...
    if entry_pos == 2:  # 'long' -> 2
        # switch
        profit[switch_index] = profit[switch_index] + switch_profit \
                               - leverage_ratio[switch_index] * 0.1 * enter_ratio[switch_index]
        pos_list[switch_index] = 2  # 'long' -> 2

        # take
        profit[take_index] = profit[take_index] + take_profit \
                             - leverage_ratio[take_index] * 0.1 * enter_ratio[take_index]
    elif entry_pos == 1:  # 'short' -> 1
        # switch
        profit[switch_index] = profit[switch_index] - switch_profit \
                               - leverage_ratio[switch_index] * 0.1 * enter_ratio[switch_index]
...
def calculate_hold(hold_prob, pos_list, price_list, leverage_ratio, enter_ratio, profit,
                   entry_pos, curr_close, additional_count, min_enter_ratio=0.05):
    # indices in hold_prob
    index = torch.tensor([0, 1], device=hold_prob.device)
    logit = torch.argmax(hold_prob[:, index], dim=1)
    hold_index = torch.where(logit == 0)[0]
    enter_index = torch.where(logit == 1)[0]
...
def calculate_now_profit(pos_list, price_list, leverage_ratio, enter_ratio, curr_price):
    now_profit = torch.zeros_like(pos_list, dtype=torch.float32, device=pos_list.device)
    short_index = torch.where(pos_list == 1)[0]  # 'short' -> 1
    long_index = torch.where(pos_list == 2)[0]  # 'long' -> 2

    short_profit = (-((curr_price - price_list[short_index]) 
                    / price_list[short_index] * 100.) * leverage_ratio[short_index]) \
                    - 0.1 * leverage_ratio[short_index] * enter_ratio[short_index]
    long_profit = (((curr_price - price_list[long_index]) 
                   / price_list[long_index] * 100.) * leverage_ratio[long_index]) \
                   - 0.1 * leverage_ratio[long_index] * enter_ratio[long_index]
...
#     for data_cnt, (entry_index, entry_pos) in enumerate(zip(entry_index_list, entry_pos_list_int)):
...
def fitness_fn(prescriptor,
               ...
               minimum_date=40):
    """
    Simulates automatic trading of Bitcoin futures. 
    Now adapted to run on GPU if available.
    """

    # Prepare simulation length (days)
    if stop_cnt != 1e9:
        simulation_date = days_difference(
            data.iloc[entry_index_list[start_data_cnt]]['Open time'],
            data.iloc[entry_index_list[int(stop_cnt)]]['Open time']
        )
    else:
        simulation_date = days_difference(
            data.iloc[entry_index_list[start_data_cnt]]['Open time'],
            data.iloc[-1]['Open time']
        )

    # Initialize state tensors on the chosen device
    pos_list = torch.zeros(chromosomes_size, dtype=torch.long, device=device)  # 0: 'hold'
    price_list = torch.full((chromosomes_size,), -1.0, dtype=torch.float32, device=device)
    leverage_ratio = torch.full((chromosomes_size,), -1, dtype=torch.int, device=device)
    enter_ratio = torch.full((chromosomes_size,), -1.0, dtype=torch.float32, device=device)
    ...
    for data_cnt, (entry_index, entry_pos) in enumerate(zip(entry_index_list, entry_pos_list_int)):
        ...
        # Convert entry_pos to a tensor on device
        entry_pos = torch.tensor(entry_pos, device=device).long()

        # Extract price info from data, as GPU tensors
        row = data.iloc[entry_index]
        curr_open = torch.tensor(row['Open'], dtype=torch.float32, device=device)
        curr_close = torch.tensor(row['Close'], dtype=torch.float32, device=device)
        curr_high = torch.tensor(row['High'], dtype=torch.float32, device=device)
        curr_low = torch.tensor(row['Low'], dtype=torch.float32, device=device)
        upper = torch.tensor(row[f'Upper_BB_{window_size}'], dtype=torch.float32, device=device)
        lower = torch.tensor(row[f'Lower_BB_{window_size}'], dtype=torch.float32, device=device)
        
        # Obtain history slice
        history_x = data.iloc[before_index+1 : entry_index+1]
        history_high = torch.tensor(history_x['High'].max(), dtype=torch.float32, device=device)
        history_low = torch.tensor(history_x['Low'].min(), dtype=torch.float32, device=device)
        ...
        # probs has shape [?, time_steps], so we index carefully
        # Move prob slice to the same device
        prob = torch.tensor(probs[:, data_cnt - skip_data_cnt], device=device)

        # Identify which positions are same/diff/hold
        hold_pos = torch.where(pos_list == 0)[0]
        same_pos = torch.where(pos_list == entry_pos)[0]
        diff_pos = torch.where((pos_list != entry_pos) & (pos_list != 0))[0]
        
        now_profit = calculate_now_profit(
            pos_list, price_list, leverage_ratio, enter_ratio, curr_close
        )

        # Forward pass through your model or prescriptor (this is user-defined)
        prob = after_forward(prescriptor, prob, now_profit, leverage_ratio, enter_ratio, pos_list, device=device)

        # Subset probabilities for each group
        same_prob = prob[same_pos]
        diff_prob = prob[diff_pos]
        hold_prob = prob[hold_pos]

        # Update states based on 'same' action
        pos_list[same_pos], price_list[same_pos], leverage_ratio[same_pos], \
        enter_ratio[same_pos], additional_count[same_pos], profit[same_pos] = calculate_same(
            same_prob, pos_list[same_pos], price_list[same_pos],
            leverage_ratio[same_pos], enter_ratio[same_pos], profit[same_pos],
            entry_pos, curr_close, additional_count[same_pos],
            limit
        )
        
        # Update states based on 'diff' action
        pos_list[diff_pos], price_list[diff_pos], leverage_ratio[diff_pos], \
        enter_ratio[diff_pos], additional_count[diff_pos], profit[diff_pos] = calculate_diff(
            diff_prob, pos_list[diff_pos], price_list[diff_pos],
            leverage_ratio[diff_pos], enter_ratio[diff_pos], profit[diff_pos],
            entry_pos, curr_close, additional_count[diff_pos]
        )
        
        # Update states based on 'hold' action
        pos_list[hold_pos], price_list[hold_pos], leverage_ratio[hold_pos], \
        enter_ratio[hold_pos], additional_count[hold_pos], profit[hold_pos] = calculate_hold(
            hold_prob, pos_list[hold_pos], price_list[hold_pos],
            leverage_ratio[hold_pos], enter_ratio[hold_pos], profit[hold_pos],
            entry_pos, curr_close, additional_count[hold_pos]
        )
        
        before_index = entry_index

        # Save snapshot of profit to CPU for analysis
        returns_list.append(profit.clone().cpu().detach().numpy())

        # Reset profit for the next iteration (if your logic requires it)
        profit = torch.zeros(chromosomes_size, dtype=torch.float32, device=device)

    # Convert returns_list to a NumPy array
    returns_list = np.array(returns_list).T  # Shape: (chromosomes_size, time_steps)

    # Calculate performance metrics (may need CPU for NumPy ops)
    metrics = calculate_performance_metrics(returns_list, minimum_date=minimum_date)
    ...
    for gen_idx in range(start_gen, gen_loop):
        ...
        if warming_step <= gen_idx:
            if gen_idx != 0:
                ...
                valid_index = np.where((train_metrics[:elite_size][:, 6] > 3))
