...
def intersect1d_torch(a, b):
    """
    Returns sorted unique values that are in both a and b (both 1D integer tensors).
    """
    a_np = a.cpu().numpy()
    b_np = b.cpu().numpy()
    # Use np.intersect1d
    c_np = np.intersect1d(a_np, b_np)
...
def calculate_action(
    ...
    if len(flip_short_to_long_idx) > 0:
        ...
        leverage_ratio[flip_short_to_long_idx] = torch.clamp(leverage_ratio[flip_short_to_long_idx], 1, 125)
    ...
    if len(flip_long_to_short_idx) > 0:
        ...
        leverage_ratio[flip_long_to_short_idx] = torch.clamp(leverage_ratio[flip_long_to_short_idx], 1, 125)
    ...
    if len(open_short_idx) > 0:
        ...
        leverage_ratio[open_short_idx] = torch.clamp(leverage_ratio[open_short_idx], 1, 125)
    ...
    if len(open_long_idx) > 0:
        ...
        leverage_ratio[open_long_idx] = torch.clamp(leverage_ratio[open_long_idx], 1, 125)
...
def fitness_fn(
    ...
    """
    GA 학습을 위해 시뮬레이션을 수행하여 각 chromosome의 매매 성능을 측정하는 함수.
    tracking_info가 주어지면, 매 스텝마다 포지션, 레버리지, 진입 비율, 현재 수익 등을 저장.

    Args:
        ...
        tracking_info (dict, optional): 매매 시뮬 정보를 저장할 딕셔너리.
            예시:
            {
              'iteration': [],
              'pos_list': [],
    ...
    if stop_cnt != 1e9:
        simulation_date = days_difference(
            ...
            data.iloc[entry_index_list[stop_cnt]]['Open time']
        )
    else:
        simulation_date = days_difference(
            data.iloc[entry_index_list[start_data_cnt]]['Open time'],
    ...
    # <--- ADD HERE: track cumulative_return step by step
    cumulative_return = torch.ones((chromosomes_size,), dtype=torch.float32, device=device)
    ...
    tracking_info = {
        ...
        'additional_count': [],
        'now_profit': [],
        'cumulative_returns': []  # <--- ADD HERE
    }

    # entry positions 문자열 -> 숫자
    ...
    # 시뮬레이션 루프
    # ---------------------------
    for data_cnt, (entry_index, entry_pos) in tqdm(
        enumerate(zip(entry_index_list, entry_pos_list_int)), 
        ...
        # 2) 현재 chromosome들이 만들어낸 확률(prob)을 가져옴
        prob = torch.tensor(probs[:, data_cnt - skip_data_cnt]).to(device)
        
        # 3) 현재 시점 미실현수익(now_profit) 계산
        now_profit = calculate_now_profit(
            pos_list, price_list, leverage_ratio, 
            enter_ratio, curr_close
        )

        # 4) after_forward로 최종 action probability 수정
        prob = after_forward(
            prescriptor, 
            prob, 
            now_profit, 
        ...
        # 6) 이번 스텝 '실현수익(profit)'을 기록 (지금은 step 기준).
        #    returns_list에 현재 step의 profit만 저장.
        returns_list.append(profit.clone().cpu().detach().numpy())

        # <--- ADD HERE: update cumulative_return (in the same manner as "calculate_performance_metrics")
        # profit is in '%'. So we do: cumulative_return *= (1 + profit/100.)
        cumulative_return *= (1.0 + (profit / 100.0))

        # 7) tracking_info 저장
        step_index = data_cnt  # or entry_index
        tracking_info['iteration'].append(step_index)
        ...
        for key, tensor in zip(
            [
                ...
                'additional_count', 
                'now_profit',
                # <--- also store the updated "cumulative_return"
            ],
            [
                pos_list, 
                price_list, 
            ...
            tracking_info[key].append(tensor.clone().cpu().numpy())
        
        # <--- Append "cumulative_return" to tracking_info
        tracking_info['cumulative_returns'].append(cumulative_return.clone().cpu().numpy())

        # (기존 코드) profit을 매 스텝마다 0으로 초기화 (step별 성능만 본다면)
        profit = torch.zeros(chromosomes_size, dtype=torch.float32, device=device)
