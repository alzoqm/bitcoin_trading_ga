...
def fitness_fn(prescriptor, data, probs, entry_index_list, entry_pos_list, skip_data_cnt, start_data_cnt, chromosomes_size, window_size,
    ...
    - 최종적으로 각 chromosome별 성과 지표(metrics)를 계산하여 numpy array와 함께 시뮬레이션 상황(tracking_info)을 반환합니다.
    """
    # 초기 포지션 및 상태 변수들
    ...
    # tracking_info 딕셔너리: 시뮬레이션 각 단계별 상태를 저장
    tracking_info = {
        'iteration': [],
        'pos_list': [],
        'price_list': [],
    ...
    for data_cnt, (entry_index, entry_pos) in tqdm(enumerate(zip(entry_index_list, entry_pos_list_int)),
        ...
        # -------------------------------
        # tracking_info 업데이트: 현재 시뮬레이션 상태 기록
        tracking_info['iteration'].append(data_cnt)
        tracking_info['pos_list'].append(pos_list.cpu().clone().numpy())
        tracking_info['price_list'].append(price_list.cpu().clone().numpy())
        tracking_info['leverage_ratio'].append(leverage_ratio.cpu().clone().numpy())
        tracking_info['enter_ratio'].append(enter_ratio.cpu().clone().numpy())
        tracking_info['profit'].append(profit.cpu().clone().numpy())
        tracking_info['additional_count'].append(additional_count.cpu().clone().numpy())
        tracking_info['now_profit'].append(now_profit.cpu().clone().numpy())
        tracking_info['cumulative_returns'].append(compound_value.cpu().clone().numpy())
        # -------------------------------
