...
def fitness_fn(prescriptor, data, probs, entry_index_list, entry_pos_list, skip_data_cnt, start_data_cnt, chromosomes_size, window_size,
    ...
    - 최종적으로 각 chromosome별 성과 지표(metrics)를 계산하여 numpy array와 tracking_info dict를 반환합니다.
    """
    ...
    for data_cnt, (entry_index, entry_pos) in tqdm(enumerate(zip(entry_index_list, entry_pos_list_int)),
        ...
        # 현재 반복에서의 상태 정보를 tracking_info에 저장 (반드시 clone하여 저장)
        now_profit_post = calculate_now_profit(pos_list, price_list, leverage_ratio, enter_ratio, curr_close)
        tracking_info['iteration'].append(data_cnt)
        tracking_info['pos_list'].append(pos_list.cpu().numpy().copy())
        tracking_info['price_list'].append(price_list.cpu().numpy().copy())
        tracking_info['leverage_ratio'].append(leverage_ratio.cpu().numpy().copy())
        tracking_info['enter_ratio'].append(enter_ratio.cpu().numpy().copy())
        tracking_info['profit'].append(profit.cpu().numpy().copy())
        tracking_info['additional_count'].append(additional_count.cpu().numpy().copy())
        tracking_info['now_profit'].append(now_profit_post.cpu().numpy().copy())
        tracking_info['cumulative_returns'].append(compound_value.cpu().numpy().copy())
