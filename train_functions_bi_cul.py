...
def calculate_performance_metrics(returns_list, minimum_date=40):
    ...
    cumulative_returns = np.full(chromosomes_size, -1e9)
    risk_free_rate = 0.0
...
def loss_cut_fn(pos_list, price_list, leverage_ratio, enter_ratio, profit,
                curr_low, curr_high, additional_count, alpha=1., cut_percent=80.):
    # 기존 고정 손절 조건 (cut_percent)를 적용한 함수 (필요시 TP/SL와 함께 병행)
...
def tp_sl_exit_fn(pos_list, price_list, stop_loss_list, take_profit_list, profit,
                  curr_high, curr_low, curr_close, leverage_ratio, enter_ratio, additional_count):
    # 롱 포지션 (pos == 2)
    long_idx = torch.where(pos_list == 2)[0]
    if len(long_idx) > 0:
        # 손절: 현재 저가가 (진입가 * (1 - stop_loss/100)) 이하인 경우
        long_sl_idx = long_idx[torch.where(curr_low <= price_list[long_idx] * (1 - stop_loss_list[long_idx] / 100.0))[0]]
        if len(long_sl_idx) > 0:
            realized_pnl = (curr_close - price_list[long_sl_idx]) / price_list[long_sl_idx] * 100.0 * leverage_ratio[long_sl_idx] * enter_ratio[long_sl_idx]
            fee = 0.1 * leverage_ratio[long_sl_idx] * enter_ratio[long_sl_idx]
            profit[long_sl_idx] += (realized_pnl - fee)
            pos_list[long_sl_idx] = 0
            price_list[long_sl_idx] = -1.0
            leverage_ratio[long_sl_idx] = -1
            enter_ratio[long_sl_idx] = -1.0
            additional_count[long_sl_idx] = 0
            stop_loss_list[long_sl_idx] = -1.0
            take_profit_list[long_sl_idx] = -1.0
        # 익절: 현재 고가가 (진입가 * (1 + take_profit/100)) 이상인 경우
        long_tp_idx = long_idx[torch.where(curr_high >= price_list[long_idx] * (1 + take_profit_list[long_idx] / 100.0))[0]]
        if len(long_tp_idx) > 0:
            realized_pnl = (curr_close - price_list[long_tp_idx]) / price_list[long_tp_idx] * 100.0 * leverage_ratio[long_tp_idx] * enter_ratio[long_tp_idx]
            fee = 0.1 * leverage_ratio[long_tp_idx] * enter_ratio[long_tp_idx]
            profit[long_tp_idx] += (realized_pnl - fee)
            pos_list[long_tp_idx] = 0
            price_list[long_tp_idx] = -1.0
            leverage_ratio[long_tp_idx] = -1
            enter_ratio[long_tp_idx] = -1.0
            additional_count[long_tp_idx] = 0
            stop_loss_list[long_tp_idx] = -1.0
            take_profit_list[long_tp_idx] = -1.0

    # 숏 포지션 (pos == 1)
    short_idx = torch.where(pos_list == 1)[0]
    if len(short_idx) > 0:
        # 손절: 현재 고가가 (진입가 * (1 + stop_loss/100)) 이상인 경우
        short_sl_idx = short_idx[torch.where(curr_high >= price_list[short_idx] * (1 + stop_loss_list[short_idx] / 100.0))[0]]
        if len(short_sl_idx) > 0:
            realized_pnl = (price_list[short_sl_idx] - curr_close) / price_list[short_sl_idx] * 100.0 * leverage_ratio[short_sl_idx] * enter_ratio[short_sl_idx]
            fee = 0.1 * leverage_ratio[short_sl_idx] * enter_ratio[short_sl_idx]
            profit[short_sl_idx] += (realized_pnl - fee)
            pos_list[short_sl_idx] = 0
            price_list[short_sl_idx] = -1.0
            leverage_ratio[short_sl_idx] = -1
            enter_ratio[short_sl_idx] = -1.0
            additional_count[short_sl_idx] = 0
            stop_loss_list[short_sl_idx] = -1.0
            take_profit_list[short_sl_idx] = -1.0
        # 익절: 현재 저가가 (진입가 * (1 - take_profit/100)) 이하인 경우
        short_tp_idx = short_idx[torch.where(curr_low <= price_list[short_idx] * (1 - take_profit_list[short_idx] / 100.0))[0]]
        if len(short_tp_idx) > 0:
            realized_pnl = (price_list[short_tp_idx] - curr_close) / price_list[short_tp_idx] * 100.0 * leverage_ratio[short_tp_idx] * enter_ratio[short_tp_idx]
            fee = 0.1 * leverage_ratio[short_tp_idx] * enter_ratio[short_tp_idx]
            profit[short_tp_idx] += (realized_pnl - fee)
            pos_list[short_tp_idx] = 0
            price_list[short_tp_idx] = -1.0
            leverage_ratio[short_tp_idx] = -1
            enter_ratio[short_tp_idx] = -1.0
            additional_count[short_tp_idx] = 0
            stop_loss_list[short_tp_idx] = -1.0
            take_profit_list[short_tp_idx] = -1.0

    return pos_list, price_list, leverage_ratio, enter_ratio, additional_count, profit, stop_loss_list, take_profit_list
...
def calculate_action(
    ...
    curr_close,
    additional_count,
    stop_loss_list,      # 새로 추가된 리스트
    take_profit_list,    # 새로 추가된 리스트
    limit=2,
    cut_value=1.0,
    min_enter_ratio=0.05
):
    # 1) 이산(action) 및 연속(enter_ratio, leverage, stop_loss, take_profit) 값 분리
    action = torch.argmax(prob[:, :3], dim=1)
    raw_enter_ratio = prob[:, 3]
    raw_leverage = prob[:, 4]
    raw_stop_loss = prob[:, 5]
    raw_take_profit = prob[:, 6]
    ...
    # 손절/익절 % 값는 0~1의 범위로 나오므로 *100하여 %로 변환
    stop_loss_percent = activation_fn(raw_stop_loss) * 10
    take_profit_percent = activation_fn(raw_take_profit) * 10
    ...
    # 3) hold action일 경우 포지션 청산 (기존 로직)
    ...
    if len(close_short_idx) > 0:
        ...
        stop_loss_list[close_short_idx] = -1.0
        take_profit_list[close_short_idx] = -1.0
    ...
    if len(close_long_idx) > 0:
        ...
        stop_loss_list[close_long_idx] = -1.0
        take_profit_list[close_long_idx] = -1.0
    ...
    if len(flip_short_to_long_idx) > 0:
        ...
        # 새 손절/익절 라인 갱신
        stop_loss_list[flip_short_to_long_idx] = stop_loss_percent[flip_short_to_long_idx]
        take_profit_list[flip_short_to_long_idx] = take_profit_percent[flip_short_to_long_idx]
    ...
    if len(flip_long_to_short_idx) > 0:
        ...
        stop_loss_list[flip_long_to_short_idx] = stop_loss_percent[flip_long_to_short_idx]
        take_profit_list[flip_long_to_short_idx] = take_profit_percent[flip_long_to_short_idx]
    ...
    if len(open_short_idx) > 0:
        ...
        stop_loss_list[open_short_idx] = stop_loss_percent[open_short_idx]
        take_profit_list[open_short_idx] = take_profit_percent[open_short_idx]
    ...
    if len(add_short_idx) > 0:
        ...
        if len(can_add_idx) > 0:
            ...
            add_ratio = torch.minimum((cut_value - before_ratio), enter_enter_ratio[can_add_idx])
            ...
            # 추가 진입 시 손절/익절 라인 덮어쓰기
            stop_loss_list[can_add_idx] = stop_loss_percent[can_add_idx]
            take_profit_list[can_add_idx] = take_profit_percent[can_add_idx]
    ...
    if len(open_long_idx) > 0:
        ...
        stop_loss_list[open_long_idx] = stop_loss_percent[open_long_idx]
        take_profit_list[open_long_idx] = take_profit_percent[open_long_idx]
...
def calculate_now_profit(pos_list, price_list, leverage_ratio, enter_ratio, curr_price):
    ...
    short_index = torch.where(pos_list == 1)[0]
    long_index = torch.where(pos_list == 2)[0]
    short_profit = (-((curr_price - price_list[short_index]) / price_list[short_index] * 100.) * leverage_ratio[short_index]) - 0.1 * leverage_ratio[short_index] 
    long_profit = (((curr_price - price_list[long_index]) / price_list[long_index] * 100.) * leverage_ratio[long_index]) - 0.1 * leverage_ratio[long_index] 
...
def calculate_fitness(metrics):
    ...
    higher_is_better_list = [True, True, True, False, True]
...
def fitness_fn(prescriptor, data, probs, entry_index_list, entry_pos_list, skip_data_cnt, start_data_cnt, chromosomes_size, window_size,
    ...
    # 신규: 손절/익절 라인을 저장할 리스트 (초기값 -1)
    stop_loss_list = torch.full((chromosomes_size,), -1.0, dtype=torch.float32, device=device)
    take_profit_list = torch.full((chromosomes_size,), -1.0, dtype=torch.float32, device=device)
    ...
    for data_cnt, (entry_index, entry_pos) in tqdm(enumerate(zip(entry_index_list, entry_pos_list_int)),
        ...
        # 추가: TP/SL 조건에 따른 강제 청산
        pos_list, price_list, leverage_ratio, enter_ratio, additional_count, profit, stop_loss_list, take_profit_list = tp_sl_exit_fn(
            pos_list, price_list, stop_loss_list, take_profit_list, profit,
            curr_high, curr_low, curr_close, leverage_ratio, enter_ratio, additional_count
        )
        ...
        # calculate_action 업데이트 (추가된 stop_loss_list, take_profit_list 전달)
        pos_list, price_list, leverage_ratio, enter_ratio, additional_count, profit, stop_loss_list, take_profit_list = calculate_action(
            prob, pos_list, price_list, leverage_ratio, enter_ratio,
            profit, curr_close, additional_count, stop_loss_list, take_profit_list,
