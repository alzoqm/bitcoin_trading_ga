...
def calculate_action(
    ...
    enter_leverage = activation_fn(raw_leverage) * 100.
    ...
    if len(close_short_idx) > 0:
        realized_pnl = (price_list[close_short_idx] - curr_close) / price_list[close_short_idx] * 100.
    ...
    if len(close_long_idx) > 0:
        ...
        price_list[close_long_idx] = -1.
        leverage_ratio[close_long_idx] = -1
        enter_ratio[close_long_idx] = -1.
    ...
    if len(add_short_idx) > 0:
        ...
        if len(can_add_idx) > 0:
            ...
            after_price = (before_price*(before_ratio/(before_ratio+add_ratio)) 
